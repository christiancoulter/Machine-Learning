# -*- coding: utf-8 -*-
"""CS4347-Assighnment1-cc833.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1nWE4-_N3gZuTWcWUPiVea5Ou7_AWWBHP
...
fastest = "myfirstlist = []" #@param ["Select one of the following", "myfirstlist = []", "myfirstlist = list()"]
...
list2 = list(range(5, 10)) #Numbers from 0 to 10 (exclusive end)
...
'''
The main difference between sort() and sorted() is that sort() has no returned 
value, and sorted() returns the iterable list. Moreover, sort() changes the 
original list 'x', and sorted() returns the sorted list as an object.
...
isInclusive = "Start is inclusive end is not" #@param ["List index start and end values are inclusive", "List index start and end are non-inclusive", "Start is inclusive end is not", "End is inclusive start is not"] {allow-input: true}
...
'''
The '[3]' may refer to the fourth list and the '[8]' may refer to the
ninth element of that fourth list (assumming the index starts at zero for the
first list or element).
...
# Your code goes here
multipleof8 = list(range(8,100,8))

multipleof9 = list(range(9,100,9))

multipleof12 = list(range(12,100,12))

'''
print('multipleof8 : ', multipleof8)
print('multipleof9 : ', multipleof9)
print('multipleof12 : ', multipleof12)
'''

lcm = set(multipleof8).intersection(multipleof9, multipleof12)
print(f'LCM of 8, 9, 12 : {lcm}')
...
def getfrom(dictionary, keys):
  values = dictionary[keys] if keys in dictionary else 999 #complete your code here
...
def getfrom(dictionary, keys):
  values =  dictionary[keys] if keys in dictionary else 999 #complete code here
  dictionary.update({'z':999}) #add 999 to dictionary
...
'''
The 'continue' statement is used since the game allows the user to keep guessing
until they guess the wrong number or their time alloted expires. Replacing the 
'continue' with a 'pass' statement would just cause the next statements to occur
and not go back to the top of the loop like 'continue.' 'Pass' would just keep 
on processing normally.
...
'''
The 'break' statement terminates the current loop being executed and continues
to the next statement. In this case it breaks out of the while loop and prints
the final score.
...
attempts = 0 #initialize the counter here
goal_number = random.randint(1,101) #should be between 1 to 100 inclusiveley, don't print this, it is cheating
while(True):
  ...
  try:
    ...
    assert guess_num in range(1,101)

  except:
    print(f'Please guess a number between 1 to 100.') # Print Appropriate message here
  ...
  if guess_num > goal_number:
    print(f'Your guess is high.')
    attempts+=1
    continue
  elif guess_num < goal_number:
...
tags = [*map(tostr, tags)] # Complete this code
...
!wget https://userweb.cs.txstate.edu/~k_b459/csv
...
# Code goes below
df['avgtemperature'] = (df.temperaturemax + df.temperaturemin)/2
...
import numpy as np
array1 = np.random.random((10,10)) # now we have our list x
...
alt.Chart(weather.sample(5000)).mark_bar().encode(
  x='count(precipitation)',
  y='avgtemperature_comaprision',
  color='avgtemperature_comaprision'
)
...
import altair as alt
alt.Chart(weather.sample(5000)).mark_bar().encode(
  x=alt.X('avgtemperature', bin=True),
  y='count()',
  color = 'avgtemperature_comaprision'
)
...
base = alt.Chart(weather.sample(5000)).mark_point().encode(
  y='avgtemperature',
  color=alt.condition(interval, 'avgtemperature_comaprision', alt.value('lightgray'))
).properties(
  selection=interval
)

base.encode(x='snowfall') | base.encode(x='precipitation')
...
plt.scatter(precipitation,avgtemperature,label = 'precipitation v. avgtemperature')
plt.scatter(snowfall, avgtemperature, label = 'snowfall v. avgtemperature')
plt.scatter(avgwindspeed, avgtemperature, label = 'avgwindspeed v. avgtemperature')